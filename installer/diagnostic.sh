...
if ! ip link show tun0 >/dev/null 2>&1; then
    echo -e "  ${YELLOW}[!]${NC} VPN не поднят: интерфейса tun0 нет, конфиг не загружен или не активирован."
    echo -e "      Проверено всё, что можно проверить без туннеля — сам туннель, Kill Switch"
    echo -e "      в бою и резервирование конфигов остаются непроверенными."
    echo ""
fi
